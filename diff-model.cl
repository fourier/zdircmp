...
(defun diff-node-to-string (node)
  (let* ((string-or-nil #'(lambda (x) (if x
    ...
    (concatenate 'string "Node: " (ztree-diff-node-short-name node)
            "\n"
...
(defun files-equal (file1 file2)
  ...
  (let ((diff-output (shell-command-to-string (concatenate 'string "diff -q" " " file1 " " file2))))
...
(defun update-node (node)
  (setq wait-message
        (concatenate 'string "Updating " (ztree-diff-node-short-name node) " ..."))
